...
| ~~AC-T4.15~~ | Two `frequency` concepts (`Frequency` tenor-step vs ICMA `frequency: int`/yr), no bridge | `schedule.py` vs `day_count.py` | **CLOSED (v0.99.0, slice #7)** — `Frequency.per_year()` implemented + consumed by the ICMA fixed-leg atom (`icma_coupon_period`): returns 12/months for integer month/year steps, **raises** for BULLET/daily/weekly/28-day (those take the BUS/252 path, never ICMA), exactly as ruled. Test: `test_icma_bus252.py`. |
