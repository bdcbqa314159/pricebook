...
def brent(f: Callable[[float], float], lo: float, hi: float,
...
def newton(f: Callable[[float], float], x0: float, fprime: Callable[[float], float],
...
def least_squares(residual: Callable[[Sequence[float]], Sequence[float]],
                  x0: Sequence[float], tol: float = 1e-12, max_iter: int = 1000) -> list[float]:
    ...
    result = _least_squares(residual, list(x0), method="lm", xtol=tol, ftol=tol, max_nfev=max_iter)
