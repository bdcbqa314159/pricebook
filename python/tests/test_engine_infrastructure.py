...
class TestEngineRegistry:
    def test_price_auto(self):
        from pricebook.models.engine_registry import price, InstrumentType
        r = price(100, 100, 0.04, 0.20, 1.0)
        assert r.price > 0
        assert r.engine_type == "analytical_bs"

    def test_price_specific_engine(self):
        from pricebook.models.engine_registry import price
        r = price(100, 100, 0.04, 0.20, 1.0, engine="tree_lr")
        assert r.price > 0
        assert "tree" in r.engine_type

    def test_american(self):
        from pricebook.models.engine_registry import price, InstrumentType
        r = price(100, 100, 0.04, 0.20, 1.0, is_call=False,
                  instrument_type=InstrumentType.AMERICAN_VANILLA)
        assert r.price > 0

    def test_list_engines(self):
        from pricebook.models.engine_registry import list_engines
        engines = list_engines()
        assert "analytical" in engines
        assert "tree_lr" in engines
        assert "mc_gbm" in engines

    def test_recommendation(self):
        from pricebook.models.engine_registry import engine_recommendation, InstrumentType
        rec = engine_recommendation(InstrumentType.EUROPEAN_VANILLA)
        assert "analytical" in rec

    def test_register_custom(self):
        from pricebook.models.engine_registry import register_engine, list_engines
        register_engine("my_custom", type("Dummy", (), {}))
        assert "my_custom" in list_engines()
