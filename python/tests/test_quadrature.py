...
These exercise the unified `pricebook.numerical._integrate.integrate(...)`
through tiny per-method test-local wrappers. The previous indirection
module `pricebook.curves.quadrature` (a legacy redirect) was deleted in
T-CRV-PT1; the four convenience wrappers live here now since they are
only used by these tests.
"""
...
from pricebook.numerical._integrate import (
    integrate, IntegrationMethod, IntegrationResult as QuadratureResult,
)
