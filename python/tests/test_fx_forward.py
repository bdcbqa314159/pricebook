...
class TestForwardRate:
    ...
    def test_cip_formula(self):
        ...
        eur_curve = _flat_curve(REF, rate=0.04)  # base
        usd_curve = _flat_curve(REF, rate=0.05)  # quote
    ...
    def test_higher_quote_rate_lowers_forward(self):
        ...
        eur_curve = _flat_curve(REF, rate=0.03)
        usd_curve = _flat_curve(REF, rate=0.05)
        mat = REF + relativedelta(years=1)
        fwd = FXForward.forward_rate(SPOT, mat, eur_curve, usd_curve)
    ...
    def test_higher_base_rate_lowers_forward(self):
        ...
        eur_curve = _flat_curve(REF, rate=0.06)
        usd_curve = _flat_curve(REF, rate=0.03)
        mat = REF + relativedelta(years=1)
        fwd = FXForward.forward_rate(SPOT, mat, eur_curve, usd_curve)
        assert fwd < SPOT

    def test_equal_rates_forward_equals_spot(self):
        eur_curve = _flat_curve(REF, rate=0.05)
        usd_curve = _flat_curve(REF, rate=0.05)
        mat = REF + relativedelta(years=1)
        fwd = FXForward.forward_rate(SPOT, mat, eur_curve, usd_curve)
...
class TestPV:
    ...
    def test_pv_zero_at_forward_strike(self):
        eur_curve = _flat_curve(REF, rate=0.04)
        usd_curve = _flat_curve(REF, rate=0.05)
        mat = REF + relativedelta(years=1)
        fwd = FXForward.forward_rate(SPOT, mat, eur_curve, usd_curve)
