...
@dataclass
class TreeGreeks:
    price: float; delta: float; gamma: float; theta: float; vega: float = 0.0
    def to_dict(self): return vars(self)


def tree_greeks(tree_pricer, spot, rate, vol, T, strike, n_steps=200,
                is_call=True, is_american=False, div_yield=0.0, vol_bump=0.01):
    ...
    dt_val = 1.0/365
    theta = (tree_pricer(spot, strike, rate, vol, T-dt_val, n_steps, is_call, is_american, div_yield) - price) / dt_val if T > dt_val else 0.0
    vega = (tree_pricer(spot, strike, rate, vol+vol_bump, T, n_steps, is_call, is_american, div_yield) - price) / vol_bump
    return TreeGreeks(price, delta, gamma, theta, vega)


@dataclass
class Binomial2DResult:
    price: float; n_steps: int
    def to_dict(self): return vars(self)

def binomial_2d(S1, S2, strike, rate, vol1, vol2, rho, T, n_steps=50,
                payoff_type="spread_call", div_yield1=0.0, div_yield2=0.0):
    """Old API — preserved for backward compatibility."""
    r = solve_tree_2d(S1, S2, strike, rate, vol1, vol2, rho, T, n_steps,
                       payoff_type=payoff_type, div_yield1=div_yield1, div_yield2=div_yield2)
    return Binomial2DResult(r.price, n_steps)
