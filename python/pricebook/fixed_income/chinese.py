"""Chinese fixed income derivatives.

DR007 swaps, CGB bonds.

    from pricebook.fixed_income.chinese import (
        DR007Swap, CGBBond,
        build_cny_curve, synthetic_dr007_strip, synthetic_cgb_strip,
    )

Conventions:
- Day count: ACT/365F for DR007 swaps
- DR007: 7-day depository repo rate (PBOC)
- CGB: semi-annual ACT/ACT ICMA, T+1
- DR007 is averaged (not compounded) but we use the same telescoping pattern

References:
    PBOC (2024). People's Bank of China — DR007.
    MOF (2024). Ministry of Finance — CGB conventions.
"""
...
def synthetic_dr007_strip(reference_date: date, rate: float = 0.018,
                          n: int = 10, slope_bp: float = 3.0) -> list[dict]:
    """Synthetic DR007 OIS strip. China: ~1.8% base rate."""
    cal = get_calendar("CNY")
...
def build_cny_curve(reference_date: date, strip: list[dict]) -> DiscountCurve:
    """Bootstrap CNY discount curve. ACT/365F, log-linear."""
...
@dataclass
class DR007SwapResult:
    pv: float; par_rate: float; dv01: float; notional: float
    def to_dict(self) -> dict: return vars(self)


class DR007Swap:
    """Chinese DR007 overnight swap. Annual fixed, ACT/365F.

    DR007 is averaged (not compounded) but we use the same telescoping
    pattern (df_start - df_end) for simplicity.
    """
    ...
    def price(self, curve: DiscountCurve) -> DR007SwapResult:
        ...
        return DR007SwapResult(pv, par, abs(pv_up - pv), self.notional)

    def pv_ctx(self, ctx) -> float: return self.price(ctx.discount_curve).pv
    def to_dict(self) -> dict:
        return {"type": "dr007_swap", "start": self.start.isoformat(),
                "end": self.end.isoformat(), "fixed_rate": self.fixed_rate}


# ═══════════════════════════════════════════════════════════════
# CGB (Chinese Government Bond) — semi-annual ACT/ACT ICMA, T+1
# ═══════════════════════════════════════════════════════════════

class CGBBond:
    """Chinese Government Bond. Semi-annual ACT/ACT ICMA, T+1."""
