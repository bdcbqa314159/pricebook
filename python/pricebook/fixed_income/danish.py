"""Danish fixed income derivatives.

DESTR swaps and DGB sovereign bonds.

    from pricebook.fixed_income.danish import (
        DESTRSwap, DGBBond, build_dkk_curve, synthetic_destr_strip,
    )

Conventions:
- Day count: ACT/360 for DESTR swaps, ACT/ACT ICMA for DGB
- DESTR: Denmark Short-Term Rate (Danmarks Nationalbank)
- DGB: Danish government bonds, annual, ACT/ACT ICMA, T+2

References:
    Danmarks Nationalbank (2024). DESTR methodology.
"""
...
def synthetic_destr_strip(reference_date: date, rate: float = 0.0335,
                                 n: int = 10, slope_bp: float = 5.0) -> list[dict]:
    """Synthetic DESTR OIS strip. Danish: ~3.35% base rate."""
    cal = get_calendar("DKK")
...
def build_dkk_curve(reference_date: date, strip: list[dict]) -> DiscountCurve:
    """Bootstrap DKK discount curve. ACT/360, log-linear."""
...
class DESTRSwap:
    ...
    def price(self, curve: DiscountCurve) -> DESTRSwapResult:
        ...
        return DESTRSwapResult(pv, par, abs(pv_up - pv), self.notional)

    def pv_ctx(self, ctx) -> float: return self.price(ctx.discount_curve).pv
    def to_dict(self) -> dict:
        return {"type": "destr_swap", "start": self.start.isoformat(),
...
class DGBBond:
    ...
    def to_dict(self) -> dict:
        return {"type": "dgb", "maturity": self.maturity.isoformat(),
                "coupon": self.coupon}
