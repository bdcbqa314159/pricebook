...
from pricebook_ng.risk.greeks import bump_equity_spot, bump_equity_vol, equity_delta, equity_vega
...
def _black_inputs(strike=105.0):
    ...
    df_div = MARKET.equity_div_curves[TICKER].df(MATURITY)
    f = SPOT * df_div / df_r
...
def test_bump_equity_spot_moves_only_that_spot():
    bumped = bump_equity_spot(MARKET, TICKER, 1.0)
    assert bumped.equity_spots[TICKER] == pytest.approx(101.0, abs=1e-12)
    assert bumped.equity_vols[TICKER] == VOL              # vol untouched
    assert MARKET.equity_spots[TICKER] == SPOT            # original unchanged


def test_bump_equity_vol_moves_only_that_vol():
    bumped = bump_equity_vol(MARKET, TICKER, 0.01)
    assert bumped.equity_vols[TICKER] == pytest.approx(0.21, abs=1e-12)
    assert bumped.equity_spots[TICKER] == SPOT


def test_equity_delta_matches_analytic():
    # d(call PV)/d(spot) = quantity * DF_div(T) * N(d1)
    _, df_div, _, _, _, d1 = _black_inputs()
    assert equity_delta(_opt_priceable(), MARKET, TICKER, NUM) == pytest.approx(
        QTY * df_div * norm_cdf(d1), abs=1e-4
    )


def test_put_delta_is_negative():
    assert equity_delta(_opt_priceable(is_call=False), MARKET, TICKER, NUM) < 0.0
...
def test_equity_vega_matches_analytic():
    ...
    assert equity_vega(_opt_priceable(), MARKET, TICKER, NUM) == pytest.approx(
