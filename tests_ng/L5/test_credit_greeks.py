...
def test_buyer_credit01_positive_via_priceable():
    assert credit01(_priceable(_cds()), MARKET, NUM) > 0.0


def test_seller_credit01_is_negative_buyer():
    b = credit01(_priceable(_cds(buy=True)), MARKET, NUM)
    s = credit01(_priceable(_cds(buy=False)), MARKET, NUM)
...
def test_credit01_matches_independent_hazard_fd():
    ...
    assert credit01(_priceable(c), MARKET, NUM) == pytest.approx(manual, abs=1e-9)
