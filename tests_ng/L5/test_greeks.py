"""Risk → L5 oracle — greeks on the Pricable protocol (bump-and-reprice).

The structural fix (spine): risk lives above the engine and depends only on a
`Pricable` (`snapshot -> PV`), never on concrete product/model classes. One
...
from pricebook_ng.risk.pricable import Pricable, discounting_pricable, hull_white_pricable
...
def test_dv01_single_cashflow_matches_analytic():
    ...
    pricable = discounting_pricable(trade, DiscountingEngine(), NUM)
    t = year_fraction(D0, mat, CURVE_DC)
    analytic = -notional * t * math.exp(-RATE * t) * _ONE_BP
    assert dv01(pricable, _market(), NUM) == pytest.approx(analytic, abs=1e-6)
...
def test_dv01_bond_matches_analytic_sum():
    ...
    pricable = discounting_pricable(bond, DiscountingEngine(), NUM)
    ...
    assert dv01(pricable, _market(), NUM) == pytest.approx(analytic, abs=1e-4)


def test_dv01_is_generic_over_any_pricable():
    # risk depends only on the Pricable interface — feed it a raw closure
    notional, mat = 500_000.0, date(2030, 1, 15)
    raw = Pricable(lambda snap: notional * snap.discount_curve.df(mat))
    t = year_fraction(D0, mat, CURVE_DC)
    analytic = -notional * t * math.exp(-RATE * t) * _ONE_BP
...
def test_dv01_swaption_rebuilds_the_model():
    ...
    a, sigma = 0.05, 0.012
    pricable = hull_white_pricable(swaption, a, sigma, SwaptionEngine(), NUM)
    generic = dv01(pricable, _market(), NUM)
